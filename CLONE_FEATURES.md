...
- Market scan notes (2026-02-09, Cycle 4): ExifTool documents that TIFF metadata removal is structurally different than JPEG because TIFF stores image data in IFD0; fully erasing common tags often requires either selective tag removal or re-encoding. https://exiftool.org/faq.html
- Market scan notes (2026-02-09, Cycle 4): Dangerzone’s official docs emphasize “render to pixels in a sandbox, then rebuild a safe PDF”, which sets UX expectations for a separate "safe export" mode vs metadata stripping. https://github.com/freedomofpress/dangerzone
- Market scan notes (2026-02-09, Cycle 4): qpdf documents `/ID` stabilization via `--deterministic-id` for reproducible outputs; reproducibility is a common expectation in automation pipelines. https://inferiorproducts.com/docs/sharedocs/qpdf/qpdf-manual.html
